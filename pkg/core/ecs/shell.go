...
func tailContainerLogs(ctx context.Context, host, serviceName string, tailLogs int32) error {
	...
	// TODO: disable StrictHostKeyChecking
	cmd := fmt.Sprintf("sudo ssh -i /opt/gatekeeper/keys/services.pem ec2-user@%s 'docker ps --format '{{.Names}}' | grep \"%s\" | cut -d\" \" -f1 | head -1'", host, strings.ReplaceAll(serviceName, "_", "-"))
	out, err := exec.Command("bash", "-c", cmd).Output()
	if err != nil {
		return err
	}
